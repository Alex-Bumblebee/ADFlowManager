...
- **Packages - deployment results dialog crash**: the results dialog after a deployment (success or failure) was crashing with a BAML TypeConverter exception, causing the error "Error deploy" to appear instead of the results. Root cause: five `SymbolRegular` enum values used in `DeploymentResultsDialog.xaml` (`CheckmarkCircle16`, `Warning16`, `DismissCircle16`, `DocumentWarning24`, `Wrench24`) do not exist in WPF-UI 4.2.0. Replaced with valid equivalents (`CheckmarkCircle24`, `Warning24`, `DismissCircle24`, `DocumentDismiss16`, `Info24`).
- **Computers - "Check Status" button not reflecting ping result**: clicking the button ran the ping correctly but the result was never displayed. Two issues: `Computer.IsOnline` is a plain property without `INotifyPropertyChanged`, so bindings could not update; and the details panel had no UI element bound to the online status. Fixed by adding observable `IsOnline` and `HasPingResult` properties to `ComputerViewModel`, and adding an **Online / Offline** badge in the details panel that appears after the first ping check.
